...
function watchIncoming(){
  ...
  stopIncoming=onSnapshot(q,snap=>{
    snap.docChanges().forEach(ch=>{
      ...
      // Clear a stale incoming notification when the caller cancels/ends.
      if(
        incomingId===ch.doc.id &&
        ['ended','declined'].includes(d.status)
      ){
        hideIncoming();
        incomingId=null;
      }
    ...
    console.error('Incoming call listener FirebaseError:',e);
  });
}
...
onAuthStateChanged(auth,userArg=>{
  user=userArg||null;

  if(!isReal()){
    if(stopIncoming){stopIncoming();stopIncoming=null;}
    if(activeCallRef)endCall(true);
    hideIncoming();
    incomingId=null;
    return;
  }
